...
const TablaAprobacionDirector = ({solicitudesAprobacion = []}) => {
...
const solicitudesPaginadas = Array.isArray(solicitudesAprobacion) ? solicitudesAprobacion.slice((page - 1) * rowsPerPage, page * rowsPerPage) : [];
  ...
  const handleClickVisualizar = (solicitud) =>{
    ...
    router.push(`/directorCarrera/tesis/visualizarSolicitudesAprobacion/visualizarRevisiones?id=${solicitud.id}`);
  }

  const handleClickEditar = (solicitud) =>{
    router.push(`/directorCarrera/tesis/visualizarSolicitudesAprobacion/registrarAprobacion?id=${solicitud.id}`);
            ...
            {solicitudesPaginadas.map((solicitud) => (
              <TableRow key={solicitud.id}>
                ...
                <TableCell align='center' sx={{ padding: '11px' }}>
                    {solicitud.estadoAprobacion == 'EN_REVISION_POR_DIRECTOR'? (
                 <>
...
export default TablaAprobacionDirector;