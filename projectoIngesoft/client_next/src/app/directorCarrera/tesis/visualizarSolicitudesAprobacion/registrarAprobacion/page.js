...
import { Box, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, Typography, IconButton,
     MenuItem, Select, Grid, Dialog, DialogTitle, DialogContent, DialogActions,FormControl,FormControlLabel,FormLabel, Radio, RadioGroup } from '@mui/material';
...
export default function registrarAprobacionDirectorCarrera() {
  ...
  const [selectedFile, setSelectedFile] = useState(null); //Estado del archivo de revisión
  const [comments, setComments] = useState('');//Estado de los comentarios
  const [approval, setApproval] = useState('');//Estado de la aprobación o desaprobación
  ...
  const redirigirASolicitudesAprobacion = async () => {
    ...
    try {
      ...
      const nuevoEstado = approval ? "APROBADA" : "OBSERVADO_POR_DIRECTOR";
