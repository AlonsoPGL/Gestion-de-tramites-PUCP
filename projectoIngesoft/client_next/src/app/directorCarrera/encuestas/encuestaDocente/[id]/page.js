  ...
  useEffect(() => {
    const fetchEncuestas = async () => {
      try {
        ...
        const encuestasDataFiltrada = encuestasData.filter(e => e.tipo === "DOCENTE" && e.activo === true);
        ...
        if (encuestasDataFiltrada.length > 0) {
          ...
          console.warn("No se encontraron encuestas de tipo DOCENTE.");
        ...
        const respuestasPromises = encuestasData.map(async (encuesta) => {
          try {
            const res = await axios.get(
              `http://localhost:8080/preguntas/pregunta/listarTodasLasRespuestasDocente/${idDocente}/${encuesta.id_Encuesta}`
            );
    ...
    <Box sx={{ backgroundColor: 'white', height: '100vh' }}>
      ...
      >
        <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#39298b', mb: '20px' }}>
          Resultados del Docente en las Encuestas
        </Typography>
        
        {encuestaActual ? (
          <div>
            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 1}}>{encuestaActual.titulo}</Typography>
            <Box>
              {docente && 
