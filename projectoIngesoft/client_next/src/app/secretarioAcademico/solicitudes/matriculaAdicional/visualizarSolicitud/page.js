...
import TablaResumenCursosSeleccionados from 'componentesDirectorDeCarrera/matriculaAdicional/TablaResumenHorariosSeleccionados';
...
export default function VisualizarSolicitudSecretario() {
    ...
    const fetchSolicitud = async () => {
      if (id) {
        try {
          ...
          setObservacion(data.observacion !== null ? data.observacion : "No hay respuesta del director aún")
    ...
    <Box sx={{ backgroundColor: 'white', height: '100vh', marginBottom: '20px' }}>
      ...
      >
        ...
        <Box sx={{ marginLeft: '20px', marginRight: '20px', marginTop: '10px' }}>
          <TablaResumenCursosSeleccionados horarioSeleccionado={cursosAdicionales} />
        </Box>
