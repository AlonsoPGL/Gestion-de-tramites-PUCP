...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, InputLabel, MenuItem, Select, Snackbar, TextField, ToggleButton, ToggleButtonGroup, Typography } from '@mui/material';
...
export default function RegistrarEncuesta() {
  ...
  useEffect(() => {
    ...
    const loadStoredEncuesta = () => {
      const storedEncuesta = JSON.parse(localStorage.getItem('selectedEncuestaJP'));
      const storedIsEditingEncuesta = JSON.parse(localStorage.getItem('editarJP'));      
      if (encuestaEdit) {
        ...
        localStorage.setItem('selectedEncuestaJP', JSON.stringify(encuestaEdit));
        localStorage.setItem('editarJP', JSON.stringify(true));
  ...
  useEffect(() => {
    if(encuesta?.titulo){
      ...
      localStorage.setItem("selectedEncuestaJP", JSON.stringify(encuesta));
    }
  }, [encuesta]);
  ...
  const handleGuardarEncuesta = async () => {
    ...
    else if (!encuesta.preguntas || encuesta.preguntas.length === 0) {
    ...
    try {
      ...
      const endpoint = isEditingEncuesta ? `http://localhost:8080/preguntas/encuesta/actualizar/${encuesta.idEncuesta}` : 'http://localhost:8080/preguntas/encuesta/insertarJP';
      ...
      if (!encuesta.idEncuesta) {
        // Si no había ID, significa que es una nueva encuesta, por lo que debemos guardar el ID recibido del backend
        setEncuesta(prevEncuesta => ({
          ...prevEncuesta,
          idEncuesta: response.data.idEncuesta,  // Suponiendo que el backend devuelve el ID
        }));
      }
      if (!encuesta) {
  ...
  const handleAisgnarEncuesta = async () => {
    if(!encuesta){
      setSnackbarMessage('Primero Debe registrar la encuesta');
      setOpenSnackbar(true);
      return;
    }
    try {
      const idEncuesta = parseInt(encuesta.idEncuesta, 10);
      await axios.post('http://localhost:8080/preguntas/encuesta/asignarHorariosEncuestaJP',idEncuesta, { headers: { 'Content-Type': 'application/json' } }); // Cambia la URL según tu API
    ...
    <Box sx={{ backgroundColor: 'white' }}>
      ...
      >
        ...
        <Typography variant="h4" sx={{ mb: '20px', color: '#191D23' }}>
          {isEditingEncuesta ? "Editar Encuesta Jefe de Práctica" : "Registro Encuesta Jefe de Práctica"}
        </Typography>
      ...
      <EstaSeguroAccion
          ...
          texto={isEditingEncuesta ? `¿Está seguro que desea editar la encuesta?` : `¿Está seguro de agregar la encuesta?`}
          handleAceptar={async () => {
...
<RegistradoConExito
          open={modalOpen2}
          onClose={() => router.push('../encuestaJP/gestionEncuestaJP')}
          texto={isEditingEncuesta ? `Encuesta Editada Con Éxito` : `Encuesta Registrada Con Éxito`}
        />

        <RegistradoConExito
          open={modalOpen3}
          onClose={() => router.push('../encuestaDocente/gestionEncuestaDocente')}
          texto={`Encuesta Asignada Con Éxito`}
        />
