...
export default function GestionEncuesta() {
  ...
  //SE AGREGA ESTADO PARA LA ASIGNACION DE ENCUESTAS
  useEffect(() => {
    const cargarEncuestas = async () => {
      try {
        // Asignación directa del contenido de las encuestas
        ...
        const encuestasData = response.data.content; // Correctamente definida y usada aquí
  
        // Verificar asignación de encuestas
        const idsEncuestas = encuestasData.map(encuesta => encuesta.idEncuesta);
        const asignacionResponse = await axios.post('http://localhost:8080/preguntas/encuesta/verificarAsignaciones', idsEncuestas);
  
        // Crear la lista de encuestas con estado de asignación
        const encuestasConEstado = encuestasData.map(encuesta => ({
          ...encuesta,
          asignada: asignacionResponse.data[encuesta.idEncuesta] || false,
        }));
  
        // Actualizar el estado con la nueva lista de encuestas
        setEncuestas(encuestasConEstado);
        setFilteredEncuestas(encuestasConEstado);
  ...
  const handleEliminarEncuesta = (idEncuesta) => {
    setEncuestaAEliminar(idEncuesta); // Guarda el ID de la encuesta a eliminar
    setModalOpen(true); // Abre el modal
  };

  const eliminarEncuesta = async () => {
    console.log("idE",encuestaAEliminar);
    if (encuestaAEliminar) {
      try {
        ...
        setEncuestas((prevEncuestas) => prevEncuestas.filter(encuesta => encuesta.idEncuesta !== encuestaAEliminar));
        setFilteredEncuestas((prevFiltered) => prevFiltered.filter(encuesta => encuesta.idEncuesta !== encuestaAEliminar));
