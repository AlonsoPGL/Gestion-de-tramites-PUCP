...
export default function SolicitudCartaPresentacion() {
  ...
  const handleViewClick = (solicitud) => {
    ...
    router.push(`/alumno/solicitudes/solicitudCartaPresentacion/visualizarCarta?id=${solicitud.id}&fechaCreacion=${solicitud.fechaCreacion}&cursoNombre=${solicitud.curso.nombre}&observacion=${solicitud.actividadesDesarrollar}&profesorNombre=${solicitud.profesor.nombre}&estado=${solicitud.estado}&integrantes=${encodeURIComponent(integrantes)}`);
};
  ...
  useEffect(() => {
    ...
    if (persona && persona.id) {
      const fetchSolicitudes = async (idPersona) => {
        try {
          const response = await fetch(`http://localhost:8080/solicitudes/carta/listar/${idPersona}`);
            ...
            <Table>
              <TableHead sx={{ backgroundColor: '#363581', color: 'white' }}>
                <TableRow>
                  ...
                  <TableCell sx={{ color: 'white' }}>Profesor</TableCell>
