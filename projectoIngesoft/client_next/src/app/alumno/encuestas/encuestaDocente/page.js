...
const TablaEncuestaDocente = () => {
  ...
  const filteredHorarios = horarios.filter((horario) =>
    ...
    horario.docentes?.some((docente) => docente.nombre?.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const rendirEncuesta = () => {
    ...
    router.push(`/alumno/encuestas/encuestaDocente/${persona.id}?idHorario=${selectedHorario}&idDocente=${selectedDocente}`);
  };

  const handleRadioChange = (horarioId, docenteId) => {
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableHead>
              <TableRow sx={{ backgroundColor: "#363581", color: "#FFFFFF" }}>
                ...
                <TableCell align="center" sx={{ color: "#FFFFFF" }}>Docente</TableCell>
            ...
            <TableBody>
              {filteredHorarios.slice((page - 1) * rowsPerPage, page * rowsPerPage).map((horario) =>
                horario.docentes && Array.isArray(horario.docentes) ? (
                  horario.docentes.map((docente) => (
                    <TableRow key={`${horario.idHorario}-${docente.id}`}>
                      ...
                      <TableCell align="center">
                        <input
                          type="radio"
                          name="horario"
                          checked={selectedHorario === horario.idHorario && selectedDocente === docente.id}
                          onChange={() => handleRadioChange(horario.idHorario, docente.id)}
                        />
                      </TableCell>
                    </TableRow>
                  ))
...
export default TablaEncuestaDocente;