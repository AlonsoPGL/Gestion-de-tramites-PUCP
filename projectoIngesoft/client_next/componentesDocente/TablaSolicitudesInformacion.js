...
export default function TablaSolicitudesInformacion({ idAluXH }) {
  ...
  const handleEditarSolicitud = (solicitud) => {
    // Navega a la página de edición con el id de la solicitud
    router.push(`/docente/alumnosEnRiesgo/solicitudinformacion/editar?idSolicitud=${solicitud.id}`);
  };

  const handleVerDetallesSolicitud = (solicitud) => {
    // Navega a la página de visualización con el id de la solicitud
    router.push(`/docente/alumnosEnRiesgo/solicitudinformacion/visualizar?idSolicitud=${solicitud.id}`);
  };
            ...
            solicitudesOrdenadas.slice((page - 1) * rowsPerPage, page * rowsPerPage).map((solicitud) => {
              ...
              return (
                <TableRow key={solicitud.id} sx={{ backgroundColor: '#F8F9FA' }}>
                  ...
                  <TableCell align='center'>
                    <IconButton onClick={() => handleEditarSolicitud(solicitud)}
                      disabled={solicitud.leido}
                    >
                      <EditIcon sx={{ fontSize: 25, cursor: 'pointer', marginRight: '5px', color: solicitud.leido ? '#9e9e9e' : '#363581'}} />
                    </IconButton>
