...
const TablaEncuestas = ({ encuestas, onDelete }) => {
  ...
  const [selectedEncuesta, setSelectedEncuesta] = useState(null);
    ...
    //Guardamos el objeto Encuesta en un context para usarlo en el editar encuesta(se adapta el ingresar encuesta) 
    const editarEncuesta = (encuesta) => {
        console.log("Que hay",encuesta)
        setEncuesta(encuesta);
        router.push('/asistenteSeccion/encuestaJP/registrarEncuestaJP');
    };
      ...
      if (fechaFin < today) {
        ...
        return;  
          ...
          try {
            ...
            await axios.post('http://localhost:8080/preguntas/encuesta/asignarHorariosEncuestaJP',idEncuesta, { headers: { 'Content-Type': 'application/json' } });
